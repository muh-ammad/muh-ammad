...
export class DistinctionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.distinctionService
      ...
      .subscribe(
        (res: HttpResponse<IDistinction[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInDistinctions();
  }
  ...
  registerChangeInDistinctions(): void {
    this.eventSubscriber = this.eventManager.subscribe('distinctionListModification', () => this.loadPage());
  }

  delete(distinction: IDistinction): void {
    const modalRef = this.modalService.open(DistinctionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.distinction = distinction;
  }
